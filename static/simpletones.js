...
//This function plays sounds
  function playSound(waveType,startFreq,endTime) {
	...
	var numnotes = (arguments.length - 1)/2
	var len = arguments[2]/numnotes

	gainNode.gain.setValueAtTime(0.01, context.currentTime);
	gainNode.gain.exponentialRampToValueAtTime(0.5, context.currentTime +  len*1/8);

	for (var i = 3; i < arguments.length; i += 2) {
		if (arguments[i] == arguments[i-2]) {
			gainNode.gain.setValueAtTime(0.5, context.currentTime + arguments[i-1] + len*3/4);
			gainNode.gain.exponentialRampToValueAtTime(0.01, context.currentTime +  + arguments[i-1] + len*7/8);
			gainNode.gain.exponentialRampToValueAtTime(0.5, context.currentTime + arguments[i+1]);
		}
		...
		oscillatorNode.frequency.setValueAtTime(arguments[i], context.currentTime + arguments[i+1]);
	}
